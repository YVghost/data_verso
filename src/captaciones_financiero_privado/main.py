"""
Fuente  : Superbancos - Captaciones (DEPÓSITOS) y Colocaciones (CARTERA) Bancos Privados
...
DOWNLOAD_BASE = Path(__file__).resolve().parents[2] / "downloads" / "captaciones_financiero_privado"
...
def run(
    ...
    """
    if etl_only:
        # ETL sobre archivos ya presentes en downloads/
        if mode in ("all", "depositos"):
            paths = _resolve_paths("depositos")
            print(f"[depositos] {len(paths)} archivos encontrados")
            loader_depositos.load(paths)
        if mode in ("all", "cartera"):
            paths = _resolve_paths("cartera")
            print(f"[cartera] {len(paths)} archivos encontrados")
            loader_cartera.load(paths)
        return

    # Descarga (y opcionalmente ETL)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="ETL — Captaciones y Cartera Bancos Privados (Superbancos)"
    )
    parser.add_argument(
        "--start", type=int, default=None,
        help="Año inicial de descarga (por defecto: 2014)"
    )
    parser.add_argument(
