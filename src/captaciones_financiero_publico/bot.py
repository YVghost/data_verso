...
def download(start_year: int = None, end_year: int = None) -> pd.DataFrame:
    """Descarga y normaliza captaciones del sistema financiero público por año."""
    ...
    for year in range(s, e + 1):
        try:
            ...
            print(f"[captaciones_financiero_publico] Error año {year}: {ex}")
...
def _scrape_year(year: int) -> pd.DataFrame:
    ...
    #       extraer "BANCA PUBLICA - CAPTACIONES ENE - DIC {year}.xlsx".
