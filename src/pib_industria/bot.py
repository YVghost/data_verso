...
DOWNLOAD_DIR = Path(__file__).resolve().parents[2] / "downloads" / "pib_industria"
...
def download(start_year: int = None, end_year: int = None) -> pd.DataFrame:
    """Descarga, normaliza y filtra el PIB por industria desde Cuentas Nacionales Trimestrales."""
    raw = _scrape()
    df = _normalize(raw)
...
def _scrape() -> pd.DataFrame:
    # TODO: navegar a BCE > Cuentas Nacionales Trimestrales,
    #       descargar Excel desde la sección disponible,
    #       extraer columnas: Año, Trimestre, Nombre Industria, Valor USD Constantes.
...
def _normalize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["valor_usd_constantes"] = normalize_numeric(df["valor_usd_constantes"])
    return df
