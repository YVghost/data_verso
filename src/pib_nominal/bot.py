...
DOWNLOAD_DIR = Path(__file__).resolve().parents[2] / "downloads" / "pib_nominal"
...
def download(start_year: int = None, end_year: int = None) -> pd.DataFrame:
    """Descarga, normaliza y filtra el PIB nominal trimestral."""
    raw = _scrape()
    df = _normalize(raw)
...
def _scrape() -> pd.DataFrame:
    # TODO: navegar al panel BCE CNT, Descargas > Datos > Exportar,
    #       leer Excel "PUB - PIB Trimestral", hoja "SeriesCT1",
    #       columnas: Año, Trimestre, PIB Constante.
...
def _normalize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["pib_constante_usd"] = normalize_numeric(df["pib_constante_usd"])
    return df
