...
COLS_NEEDED = ["CODIGO_SECTOR_N1", "MES", "TOTAL_VENTAS"]


def download(start_year: int = None, end_year: int = None) -> pd.DataFrame:
    """Descarga y normaliza ventas por actividad económica (SRI) por año."""
    ...
    for year in range(s, e + 1):
        try:
            ...
            print(f"[ventas_actividad_economica_sri] Error año {year}: {ex}")
    return pd.concat(frames, ignore_index=True) if frames else pd.DataFrame()


def _scrape_year(year: int) -> pd.DataFrame:
    # TODO: navegar a SRI datasets > Ventas-Compras >
    #       "Total de ventas y compras mensualmente (provincia y actividad económica)",
...
def _normalize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["total_ventas"] = normalize_numeric(df["total_ventas"])
    return df
